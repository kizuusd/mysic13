document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderPlaylist() {
        ...
        document.querySelectorAll('.track-item').forEach(item => {
            item.addEventListener('click', function(e) {
                // Hindari klik pada button yang sudah punya event sendiri
    ...
    function loadTrack(index) {
        ...
        if (!track.audioSrc) {
            ...
            currentTrackTitle.textContent = `${track.title} (Demo)`;
            currentTrackArtist.textContent = `${track.artist} - Hanya Simulasi Player`;
    ...
    // Fungsi untuk mensimulasikan pemutaran track
    function playTrack() {
        if (!tracks.length) return;
        
        const track = tracks[currentTrackIndex];
        
        // Jika tidak ada audio source, gunakan simulasi
        if (!track.audioSrc || track.audioSrc === '') {
            simulateAudioPlayback();
            return;
        }
        
        // Normal playback jika audio tersedia
    ...
    let simulatedDuration = 180; // 3 menit dalam detik
    ...
    const playlistContainer = document.getElementById('playlist-container');
    if (playlistContainer && (!tracks || tracks.length === 0)) {
        playlistContainer.innerHTML = `
            <div class="loading-indicator">
                <i class="fas fa-music"></i>
                <p>Selamat datang di Mysic!</p>
                <p class="small">Musik Player Sederhana Tanpa API Eksternal</p>
            </div>
        `;
    }
}); 